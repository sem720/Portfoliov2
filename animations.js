...
class Particle {
  ...
  draw() {
    ...
    this.rotationVel(this.rotation);
  ...
  interact(other) {
    ...
    if (d < spacing) {
      ...
      if (overlap > 0) {
        let correctionsStrenght = map(overlap, 0, spacing, 0.15, 0.25);
        let correction = force.copy().mult(overlap * correctionsStrenght);
